...
def synolog_chat24(title: str, content: str) -> None:
    """
    通过 群晖chat2.4 推送消息。
    """
    if not push_config.get("SYNOLOG_CHAT24_URL"):
        print("自定义通知的 SYNOLOG_CHAT24_URL 未设置!!\n取消推送")
        return

    print("自定义通知服务启动")
    data = '{"text": "'+content+'"}'
    SYNOLOG_CHAT24_URL = push_config.get("SYNOLOG_CHAT24_URL")
    body = {
        "payload":data.encode('utf-8')
        } 
    response = requests.request(
        method='POST', url=SYNOLOG_CHAT24_URL, timeout=15, data=body
    )
    # print(body)
    # print(response.text)
    if response.status_code == 200:
...
if push_config.get("SYNOLOG_CHAT24_URL"):
    notify_function.append(synolog_chat24)
